...
namespace TaskManager.DataAccess.Repository.Concrete
{
    public class MongoRepositoryBase<T> : IRepository<T> where T : class,new()
    {
        ...
        /// <summary>
        /// Async Yapı! Liste halinde task modelini user'a ekler.
        /// </summary>
        /// <param name="items">Model alır</param>
        /// <returns></returns>
        public async Task<GetManyResult<Tasks>> AddManyAsync(ICollection<Tasks> entities)
        {
            var result = new GetManyResult<Tasks>();
            try
            {
                await _collection.InsertManyAsync((IEnumerable<T>)entities);
                result.Result = entities;
            }
            catch (Exception ex)
            {
